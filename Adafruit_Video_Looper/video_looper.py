...
class VideoLooper:
    ...
    def _load_file_reader(self):
        ...
        if getattr(self, '_read_only', False) and module in ('usb_drive', 'usb_drive_copymode'):
            # TABARC: read-only mode should not be mounting/copying anything. Directory reader only.
            module = 'directory'
...
def _warn_if_not_target_hardware(self):
    """Print a one-time warning if running on boards outside the target class.

    TABARC note:
    - People will run this on Pi 4/5 anyway.
    - It might work. It's just not what this project optimises for.
    """
    try:
        ...
        model = ''
    if model and ('Raspberry Pi 4' in model or 'Raspberry Pi 5' in model):
        self._print('warning: detected {0}. This repo targets Pi Zero/1/2/3 class hardware.'.format(model))


def _touch_alive(self, now_ts: float) -> None:
    if not self._alive_file:
        return
    try:
        # Keep this cheap: a single overwrite of a tiny file.
        with open(self._alive_file, 'w') as f:
            f.write(str(int(now_ts)))
    except Exception:
        # If we can't write it, shrug and continue.
        pass


def _preflight_ok(self, path: str) -> bool:
    ...
    try:
        ...
        if os.path.getsize(path) <= 0:
            return False
        # Very cheap extension check. Not perfect, but catches obvious junk.
        ext = os.path.splitext(path)[1].lower()
        return ext in ('.mp4', '.m4v', '.mov', '.avi', '.mkv', '.jpg', '.jpeg', '.png', '.gif')
    except Exception:
        return False
    ...
    def run(self):
        ...
        # TABARC: failure backoff + restart cooldown.
        failure_streak = 0
        restart_streak = 0
        ...
        while self._running:
            ...
            if not self._player.is_playing() and not self._playbackStopped:
                if movie is not None: #just to avoid errors
                    ...
                    # Preflight: skip obvious junk quickly.
                    if not self._preflight_ok(movie):
                        self._print('skipping invalid media: 0'.format(movie))
                        movie = self._playlist.get_next(self._is_random, self._resume_playlist)
                        continue

                    now_ts = time.time()
                    if now_ts - last_alive >= max(1, self._alive_interval_sec):
                        self._touch_alive(now_ts)
                        last_alive = now_ts

                    # Watchdog-ish behaviour: if playback keeps failing/stopping too quickly,
                    # apply a cooldown so we don't thrash the filesystem and decoder.
                    if now_ts - restart_window_start > 60:
                        restart_window_start = now_ts
                        restart_streak = 0
                    restart_streak += 1
                    if restart_streak >= 20:
                        self._print('too many restarts in a minute; cooling down for 30s')
                        time.sleep(30)
                        restart_window_start = time.time()
                        restart_streak = 0
                    ...
                    try:
                        ...
                        failure_streak += 1
                        self._print('playback start failed: 0'.format(e))
                        backoff = min(5.0, 0.25 * (2 ** min(failure_streak, 5)))
                        self._print('backing off for 0.00s (failure streak: 1)'.format(backoff, failure_streak))
                        time.sleep(backoff)
                        movie = self._playlist.get_next(self._is_random, self._resume_playlist)
                        continue
            ...
            # TABARC note:
            # Pi Zero-class boards don't need a 2ms spin-loop. That's just burning power for the illusion of responsiveness.
            # We keep a small sleep to reduce the pause between files and react to reader changes, but not at laptop-fan speed.
            sleep_s = self._wait_time if getattr(self, '_wait_time', None) else 0.05
            sleep_s = max(0.02, min(0.25, float(sleep_s)))
            time.sleep(sleep_s)
