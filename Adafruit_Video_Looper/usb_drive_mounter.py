...
# TABARC note:
# We only ever create/remove mount folders under a known root.
# If root/path logic goes wrong, `rm -r` will happily ruin your day.
def _safe_under_root(root: str, path: str) -> bool:
    try:
        root_abs = os.path.abspath(root)
        path_abs = os.path.abspath(path)
        return path_abs.startswith(root_abs.rstrip(os.sep) + os.sep)
    except Exception:
        return False
...
class USBDriveMounter:
    ...
    def remove_all(self):
        ...
        for path in glob.glob(self._root + '*'):
            ...
            if _safe_under_root(self._root, path):
...
if __name__ == '__main__':
    ...
    while True:
        if drive_mounter.poll_changes():
            ...
            drive_mounter.mount_all()
        time.sleep(0.05)  # TABARC: avoid busy waiting
